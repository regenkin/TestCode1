    ...
    public static class ToolOperate
    {
        ...
        public static string CreateTempModule(Sys_Module tempModule, DetailFormObject tempField)
        {
            ...
            foreach (string json in tempField.ModuleDatas)
            {
                ...
                foreach (NameValueObject nv in nvs)
                {
                    ...
                    if (p == null) continue;
                    p.SetValue(fieldConfig, TypeUtil.ChangeType(nv.value, p.PropertyType), null);
                }
            ...
            {
                //当前属性字段集合
                List<string> fieldNames = fieldConfigs.Select(x => x.FieldName).ToList();
                //需要删除的属性字段集合
                List<Sys_Field> tempDelSysFields = CommonOperate.GetEntities<Sys_Field>(out err, x => x.Sys_ModuleId == tempModule.Id && !fieldNames.Contains(x.Name));
                if (tempDelSysFields == null) tempDelSysFields = new List<Sys_Field>();
                List<Guid?> delSysFieldIds = tempDelSysFields.Select(x => (Guid?)x.Id).ToList();
                //删除不需要的字段
                CommonOperate.DeleteRecordsByExpression<Sys_Field>(x => delSysFieldIds.Contains(x.Id), out err);
                //删除不需要的表单字段
                Sys_Form form = SystemOperate.GetDefaultForm(tempModule.Id);
                CommonOperate.DeleteRecordsByExpression<Sys_FormField>(x => x.Sys_FormId == form.Id && delSysFieldIds.Contains(x.Sys_FieldId), out err);
                //删除不需要的列表字段
                Sys_Grid grid = SystemOperate.GetDefaultGrid(tempModule.Id);
                CommonOperate.DeleteRecordsByExpression<Sys_GridField>(x => x.Sys_GridId == grid.Id && delSysFieldIds.Contains(x.Sys_FieldId), out err);
            }
            //生成临时模块实体类
            ...
            if (string.IsNullOrEmpty(errMsg))
            {
                ...
                { 
                    "System.dll", 
                    "System.Core.dll", 
                    string.Format("{0}Kinfar.Frame.EntityBase.dll",basePath), 
                    string.Format("{0}Kinfar.Frame.Model.dll", basePath), 
        ...
        public static string GenerateModel(ModuleConfigAttribute moduleConfig, List<FieldConfigAttribute> fieldConfig, string file)
        {
            ...
            try
            {
                ...
                header.AppendLine("using Kinfar.Frame.EntityBase;");
                header.AppendLine("using Kinfar.Frame.EntityBase.Attr;");
                header.AppendLine("using Kinfar.Frame.Model.EnumSpace;");
                ...
                code.AppendLine();
                code.AppendLine("namespace Kinfar.Frame.TempModel"); //默认命名空间
                code.AppendLine("{");
                #region 开始生成类
                ...
                foreach (PropertyInfo p in moduleConfigProperties)
                {
                    ...
                    {
                        string value = p.GetValue(moduleConfig, null).ObjToStr();
                    ...
                    {
                        if (p.GetValue(moduleConfig, null).ObjToBool())
                        {
                            moduleConfigSb.AppendFormat(",{0}=true", p.Name);
                        }
                    }
                    else if (p.PropertyType == typeof(Int32))
                    {
                        int value = p.GetValue(moduleConfig, null).ObjToInt();
                        if (value > 0)
                        {
                            moduleConfigSb.AppendFormat(",{0}={1}", p.Name, value);
                        }
                ...
                foreach (FieldConfigAttribute fc in fieldConfig)
                {
                    ...
                    foreach (PropertyInfo p in fieldConfigProperties)
                    {
                        ...
                        if (p.PropertyType == typeof(String))
                        {
                            string value = p.GetValue(fc, null).ObjToStr();
                            if (!string.IsNullOrWhiteSpace(value))
                            {
                        ...
                        {
                            if (p.GetValue(fc, null).ObjToBool())
                            {
                        ...
                        {
                            int value = p.GetValue(fc, null).ObjToInt();
                            if (value > 0)
                            {
                                fieldConfigSb.AppendFormat(",{0}={1}", p.Name, value);
                            }
        ...
        /// <returns></returns>
        public static string CreateImportModelTemplate(Sys_Module module, Guid userId, string fileName)
        {
            Sys_Form form = SystemOperate.GetUserForm(userId, module.Id);
        ...
        public static IEnumerable FillModels(Guid moduleId, DataTable dt, out string errMsg, Func<object, string> action = null)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    //根据字段名获取字段
                    Sys_Field sysField = SystemOperate.GetFieldInfo(moduleId, dc.ColumnName);
                    if (sysField == null)
                    {
                        //根据字段显示名称获取字段
                        sysField = SystemOperate.GetFieldByDisplay(moduleId, dc.ColumnName);
                    }
                    if (sysField == null) continue;
                    PropertyInfo propertyInfo = modelType.GetProperty(sysField.Name);
                    if (propertyInfo == null) continue;
                    Sys_FormField field = SystemOperate.GetDefaultFormSingleField(moduleId, sysField.Name);
                    ...
                    else if (SystemOperate.IsEnumField(modelType, sysField.Name)) //枚举字段
                    {
                        ...
                        if (!dic.ContainsValue(value.ObjToStr())) //当前值不是枚举值
                        {
                            ...
                            {
                                value = 0;
                                sb.AppendLine(string.Format("第【{0}】行对应字段【{1}】的值非法，请检查该枚举字段的值！", rowNum, dc.ColumnName));
                            }
                        }
                    }
                    #endregion
                    ...
                    else if (SystemOperate.IsDictionaryBindField(moduleId, sysField.Name)) //字典字段
                    {
                        ...
                        if (dic.Select(x => x.Value).Contains(value.ObjToStr())) //当前值不是字典值
                        {
                            ...
                            {
                                value = string.Empty;
                                sb.AppendLine(string.Format("第【{0}】行对应字段【{1}】的值非法，请检查该字典字段的值！", rowNum, dc.ColumnName));
                            }
                        }
                    }
                    #endregion
                    ...
                    else if (SystemOperate.IsForeignField(sysField)) //外键字段
                    {
                        if (value.ObjToStr() != string.Empty) //外键字段值非空时
                        {
                            if (value.ObjToGuid() == Guid.Empty) //非正常值
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(foreignModule.TitleKey))
                                {
                                    ...
                                    {
                                        value = 0;
                                        sb.AppendLine(string.Format("第【{0}】行对应字段【{1}】值在外键模块【{2}】中不存在，请重新检查该字段值！", rowNum, dc.ColumnName, foreignModule.Name));
                                    }
                                    else
                                    {
                                        value = idObj;
                                ...
                                {
                                    sb.AppendLine(string.Format("第【{0}】行对应字段【{1}】的外键模块【{2}】的TitleKey值非法，无法导入该字段值！", rowNum, dc.ColumnName, foreignModule.Name));
                                }
                            }
                        }
                        else //外键字段值为空时且必填时
                        {
                            ...
                            {
                                sb.AppendLine(string.Format("第【{0}】行对应字段【{1}】为必填项！", rowNum, dc.ColumnName));
                            }
                        }
                    }
                    #endregion
                    propertyInfo.SetValue(model, TypeUtil.ChangeType(value, propertyInfo.PropertyType), null);
                }
                if (action != null)
                {
        ...
        public static DataTable FillToDataTable(Guid moduleId, object data, Guid userId, out string errMsg)
        {
            errMsg = string.Empty;
            if (data == null) return null;
            Type modelType = CommonOperate.GetModelType(moduleId);
            List<Sys_GridField> gridFieds = SystemOperate.GetUserGridFields(userId, moduleId);
            if (gridFieds == null || gridFieds.Count == 0) return null;
            gridFieds = gridFieds.Where(x => x.Sys_FieldName != "Id" && x.IsVisible && !x.IsDeleted).ToList();
            ...
            foreach (Sys_GridField gridField in gridFieds)
            {
                ...
                fieldNames.Add(gridField.Sys_FieldName, gridField.Display);
            }
            List<PropertyInfo> ps = modelType.GetProperties().Where(x => fieldNames.Keys.Contains(x.Name)).ToList();
            foreach (object obj in (data as IEnumerable))
            {
                DataRow dr = dt.NewRow();
                foreach (PropertyInfo p in ps)
                {
                    string value = SystemOperate.GetFieldDisplayValue(moduleId, obj, p.Name);
                    dr[fieldNames[p.Name]] = value;
                }
                dt.Rows.Add(dr);
            }
            return dt;
        ...
        public static string InitNewModules(List<Type> modelTypes, bool isCustomer = true)
        {
            ...
            try
            {
                ...
                foreach (Type type in modelTypes)
                {
                    ...
                    module.OtherJs = moduleConfig.OtherJs;
                    ...
                    if (moduleConfig.IsAllowAdd)
                    {
                        ...
                        gridAddButton.ButtonIcon = "eu-icon-add";
                    ...
                    if (moduleConfig.IsAllowDelete)
                    {
                        ...
                        gridDeleteButton.ButtonIcon = "eu-icon-del";
                    ...
                    else if (moduleConfig.Name == "用户管理")
                    {
                        ...
                        setRoleBtn.ButtonTagId = "btnSetForm";
                    ...
                    foreach (PropertyInfo p in properties)
                    {
                        ...
                        if (fieldAttr.IsEnableGrid)
                        {
                            ...
                            int w = p.PropertyType == typeof(Boolean) || p.PropertyType == typeof(Boolean?) ? 80 : fieldAttr.HeadWidth;
                            if (p.PropertyType == typeof(DateTime) || p.PropertyType == typeof(DateTime?))
                            {
                                w = fieldAttr.ControlType == (int)ControlTypeEnum.DateTimeBox ? 165 : 120;
                            }
                        ...
                        if (fieldAttr.IsEnableForm)
                        {
                            ...
                            formField.IsVisible = fieldAttr.IsFormVisible;
                            formField.RowNo = fieldAttr.RowNum;
                            formField.ColNo = fieldAttr.ColNum;
                            formField.AfterContent = fieldAttr.AfterContent;
        ...
        public static string InitData()
        {
            ...
            if (IsNeedInit()) //需要初始化
            {
                ...
                if (string.IsNullOrEmpty(errMsg))
                {
                    ...
                    //添加超级管理员
                    Guid adminUserId = UserOperate.AddUser(out errMsg, "admin", "admin", orgId);
                    //添加管理员角色
                    ...
                    Sys_Menu todoMenu = new Sys_Menu()
                    {
                        Name = "我的待办",
                        Display = "我的待办",
                        Url = "/Page/Grid.html?moduleName=待办任务&condition={tp:0}",
                        ParentId = BpmMenuId,
                        IsValid = true,
                        IsLeaf = true,
                        Sort = 2
                    ...
                    Sys_Menu applyMenu = new Sys_Menu()
                    {
                        Name = "我的申请",
                        Display = "我的申请",
                        Url = "/Page/Grid.html?moduleName=待办任务&condition={tp:1}",
                        ParentId = BpmMenuId,
                        IsValid = true,
                        IsLeaf = true,
                        Sort = 3
                    ...
                    Sys_Menu approvalMenu = new Sys_Menu()
                    {
                        Name = "我的审批",
                        Display = "我的审批",
                        Url = "/Page/Grid.html?moduleName=待办任务&condition={tp:2}",
                        ParentId = BpmMenuId,
                        IsValid = true,
                        IsLeaf = true,
                        Sort = 4
