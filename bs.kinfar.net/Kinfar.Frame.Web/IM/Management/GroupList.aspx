...
<head id="Head1" runat="server">
	<title></title>
	<link href=<%= Kinfar.Frame.IMCore.ServerImpl.Instance.ReplaceVariable("../{RESPATH}/Themes/Default/table.css") %>	rel="stylesheet" type="text/css" />
	...
	<script type="text/javascript" language="javascript">
		...
		window.onload = function()
		{
			...
			{
				parent.IMCore.Session.GetGlobal("FriendsInfoCache").Refresh();
			}
			else if (state.Action == "Error")
			{
				parent.IMCore.Utility.ShowError(state.Exception.Message);
			}
		}
		...
		function Update(name)
		{
		    var config = {
		        ...
		        HasMaxButton: false,
		        HasMinButton: true,
		        AnchorStyle: parent.IMCore.WindowAnchorStyle.Left |  parent.IMCore.WindowAnchorStyle.Top
		    }
		    
		    var form =  parent.IMCore.CreateWindow(config);
		    form.MoveEx('CENTER', 0, 0, true);
		    form.ShowDialog(CurrentWindow, 'CENTER', 0, 0, true, null);
		    var url =  parent.IMCore.GetPageUrl(String.format("Management/UpdateAccountInfo.aspx?random={0}&Name={1}", (new Date()).getTime(), name));
		    form.Load(url, null);
		}
