...
{
	var m_Config=IMCore.Utility.Clone(config);
...
function Control(config)
{
	...
	var _borderWidth = IMCore.Utility.IsNull(config.BorderWidth, 0);
	This.GetBorderWidth = function() { return _borderWidth; }

	var _padding = IMCore.Utility.IsNull(config.Padding, 0);
	This.GetPadding = function() { return _padding; }

	//框架
	var _dom = IMCore.Utility.IsNull(config.Dom, document.createElement("DIV"));
	...
	This.OnResized = new IMCore.Delegate();
	...
	//AnchorStyle
	var _anchorStyle = IMCore.Utility.IsNull(config.AnchorStyle, AnchorStyle.Left | AnchorStyle.Top);
	...
	This.OnDispose = new IMCore.Delegate();
	...
	IMCore.Utility.AttachEvent(
		...
		function(evt)
		{
			...
			if (IMCore.Utility.GetButton(evt) == "Right")
			{
	...
	//初始化位置和大小
	This.Move(IMCore.Utility.IsNull(config.Left, 0), IMCore.Utility.IsNull(config.Top, 0));
	This.Resize(IMCore.Utility.IsNull(config.Width, 200), IMCore.Utility.IsNull(config.Height, 150));
	This.SetCss(IMCore.Utility.IsNull(config.Css, "control"));
	This.SetParent(IMCore.Utility.IsNull(config.Parent, null));
	This.SetVisible(IMCore.Utility.IsNull(config.Visible, true));
}
...
function Label(config)
{
	...
	{
		This.GetDom().innerHTML = IMCore.Utility.ReplaceHtml(value);
	}
	...
	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.SetCss(IMCore.Utility.IsNull(config.Css, "label"));
}
...
function Button(config)
{
	...
	This.OnClick = new IMCore.Delegate();
	...
	'</tr>'+
	'</table>'
	_dom.tabIndex = IMCore.Utility.IsNull(config.TabIndex, -1);
	...
	IMCore.Utility.AttachEvent(
		...
		{
			if(evt==undefined) evt=window.event;
			if(IMCore.Utility.GetButton(evt)=="Left")
			{
	...
	IMCore.Utility.AttachEvent(
		...
		{
			if(evt==undefined) evt=window.event;
			if(IMCore.Utility.GetButton(evt)=="")
			{
	...
	This.SetText = function(value)
	{
		This.GetDom().firstChild.rows[0].cells[1].innerHTML = IMCore.Utility.ReplaceHtml(value);
		Base.SetText(value);
	}
	...
	IMCore.Utility.DisableSelect(_dom,true);

	_dom = null;

	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.SetCss(IMCore.Utility.IsNull(config.Css, "button"));
...
function TextBox(config)
{
	...
	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(IMCore.Utility.IsNull(config.Css, "textbox"));
}
...
function TextArea(config)
{
	...
	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(IMCore.Utility.IsNull(config.Css, "textarea"));
}
...
function CheckBox(config)
{
	...
	This.SetText = function(value)
	{
		_textdom.innerHTML = IMCore.Utility.ReplaceHtml(value);
	}
	...
	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(IMCore.Utility.IsNull(config.Css, "checkbox"));
}
...
function RadioButton(config)
{
	...
	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(IMCore.Utility.IsNull(config.Css, "radio"));
}
...
function DropDownList(config)
{
	...
	var _dom = This.GetDom();
	_dom.innerHTML = '<select size="1"></select>';
	...
	This.SetText(IMCore.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(IMCore.Utility.IsNull(config.Css, "select"));
}
...
function ListBox(config)
{
	...
	This.GetDropDownListDom().size = IMCore.Utility.IsNull(config.Size, 4);
...
function TreeView(config)
{
	...
	var nodeHeight = IMCore.Utility.IsNull(config.NodeHeight, 20);
	var hasImage = IMCore.Utility.IsNull(config.HasImage, false);

	var dom = This.GetDom();
	dom.innerHTML = "<div style='overflow:visible;'></div>";

	This.OnDblClick = new IMCore.Delegate();
	This.OnClick = new IMCore.Delegate();
	This.OnExpand = new IMCore.Delegate();
	This.OnCollapse = new IMCore.Delegate();
	This.OnBeginRequest = new IMCore.Delegate();
	This.OnEndRequest = new IMCore.Delegate();
	...
	*/
	This.Refresh = function(callback)
	{
				...
				function(subNodeData, error)
				{
					if (subNodeData != null)
					{
						...
						for (var i in subNodeData)
						{
							...
							var node = new TreeNode(This, null, nd.Name, nd.Text, nd.ImageCss, nd.Tag, dom.firstChild, dataSource, nodeHeight, i == subNodeData.length - 1, nd.HasChildren,nd.ImageSrc);
							rootNodes.push(node);
		...
		{
			callback(null, new IMCore.Exception(ex.name, ex.message));
		}
	}
	...
	*/
	This.Find = function(callback, path)
	{
		...
		{
			callback(null, new IMCore.Exception("Error", "路径格式不正确!"));
		}
	}
...
function TreeNode(treeView, parentNode, name, text, imgSrc, tag, container, dataSource, height, isLast, hasChildren, imagePath)
{
	...
	{
		text = val;
		textDiv.innerHTML = IMCore.Utility.ReplaceHtml(val);
	}
			...
			dataSource.GetSubNodes(
				function(subNodeData, error)
				{
					if (subNodeData != null)
					{
						...
						{
							var nd = subNodeData[i];
							var node = new TreeNode(treeView, This, nd.Name, nd.Text, nd.ImageCss, nd.Tag, subNodesContainer, dataSource, height, i == subNodeData.length - 1, nd.HasChildren, nd.ImageSrc);
							subNodes.push(node);
							nodeIndex[nd.Name] = node;
						}
						if (This.IsExpand())
	...
	*/
	...
	{
		if (This.IsExpand()) This.Collapse(); else This.Expand(function(result, error) { if (!result) IMCore.Utility.ShowError(error) });
	}

	var imgCell = nodeRow.insertCell(-1);
	if(imagePath==undefined) imgCell.innerHTML = String.format("<div class='{0}' style='width:16px; height:16px;' ></div>", imgSrc);
	else imgCell.innerHTML = String.format("<img src='{0}' style='width:16px; height:16px;' />", imagePath);
	
	This.SetImage = function(src, css)
	{
		if(src == null) imgCell.innerHTML = String.format("<div class='{0}' style='width:16px; height:16px;' ></div>", css);
		else imgCell.innerHTML = String.format("<img src='{0}' style='width:16px; height:16px;' />", src); 
	}

	var textCell = nodeRow.insertCell(-1);
	textCell.innerHTML = String.format("<div class='nodeText'>{0}</div>", IMCore.Utility.ReplaceHtml(text));
...
function ListView(config)
{
	...
	This.OnItemDblClick = new IMCore.Delegate();

	var _columns = [];
	
	var _enableMultiSelect = IMCore.Utility.IsNull(config.EnableMultiSelect, false);
	var _viewMode = IMCore.Utility.IsNull(config.ViewMode, "List");
	var _list = null;
		...
		case "List":
			{
				_list = {
					Config: {
						HeaderHeight: IMCore.Utility.IsNull(config.ListMode.HeaderHeight, 28),
						RowHeight: IMCore.Utility.IsNull(config.ListMode.RowHeight, 24)
					}
				};
	...
	This.Reset(IMCore.Utility.IsNull(config.Columns,[]));
	...
	function ListViewItem(values, css)
	{
		...
		if (_viewMode == "List")
		{
			...
			for (var i in _columns)
			{
				...
				colDom.innerHTML = IMCore.Utility.ReplaceHtml(val);
		...
		dom.onmousedown = function(evt)
		{
			...
			else if (IMCore.Utility.GetButton(evt) == "Left")
			{
				if (_enableMultiSelect)
				{
					if (evt.ctrlKey)
	...
	function ListHeader()
	{
		...
		This.OnColumnClick = new IMCore.Delegate();
		...
		This.Create=function()
		{
			...
			for (var i in _columns)
			{
				...
				colDom.style.width = col.GetWidth() + 'px';
				width += col.GetWidth();
				colDom.className = IMCore.Utility.IsNull(col.GetCss(),"td_header_default");
				colDom.innerHTML = String.format("<span>{0}</span>", IMCore.Utility.ReplaceHtml(col.GetText()));
				(function(index)
				{
	...
	function ItemPanel()
	{
		...
		This.OnScroll = new IMCore.Delegate();
...
function MenuItem(itemConfig, parent, mainMenu)
{
	...
	var obj = document.createElement("DIV");
	obj.innerHTML = IMCore.Utility.ReplaceHtml(text);
	obj.className = css;
	IMCore.Utility.AttachButtonEvent(obj, css, css+"_hover", css+"_hover");
...
function Frame(config)
{
	...
	if (window.ClientMode != undefined && !window.ClientMode && window.CurrentWindow != undefined)
	{
		IMCore.Utility.AttachEvent(
			This.GetDocument(),"mousedown",
			function()
			{
	...
	This.OnBeforeLoad = new IMCore.Delegate();
	This.OnAfterLoad = new IMCore.Delegate();
	...
	This.SetCss(IMCore.Utility.IsNull(config.Css, "frame"));
	This.Resize(This.GetWidth(), This.GetHeight());
...
function HtmlListBox(css, width)
{
	...
	IMCore.Utility.AttachEvent(
		...
		{
			if(evt==undefined) evt=window.event;
			IMCore.Utility.CancelBubble(evt);
		}
	);
	...
	this.AddItem=function(text,value)
	{
		...
		item_dom.innerHTML = IMCore.Utility.ReplaceHtml(text);
...
function HtmlDropDownList(css,width,height)
{
	...
	var dom=document.createElement("DIV");
	dom.className=IMCore.Utility.IsNull(css,"html_dropdownlist");
	...
	this.OnChanged=new IMCore.Delegate();
	...
	this.SetText=function(text)
	{
		...
		for(var i in items)
		{
			...
			{
				dom.firstChild.firstChild.innerHTML = IMCore.Utility.ReplaceHtml(text);
				This.OnChanged.Call();
			}
		}
	}
	...
	this.SetValue=function(value)
	{
		...
		for(var i in items)
		{
			...
			{
				dom.firstChild.firstChild.innerHTML = IMCore.Utility.ReplaceHtml(items[i].Text);
				This.OnChanged.Call();
			}
		}
	}
	...
	This.ShowList = function()
	{
		if(!m_IsListVisible)
		{
			var coord = IMCore.Utility.GetClientCoord(dom);
			...
			IMCore.Utility.AttachEvent(document, "click", document_onclick);
		}
	}
	...
	This.HideList = function()
	{
		if(m_IsListVisible)
		{
			...
			IMCore.Utility.DetachEvent(document, "click", document_onclick);
		}
	}
...
function Toolbar(config)
{
	...
	This.CreateControl=function(item)
	{
		if(item.Type==undefined || item.Type=="Button")
		{
			...
			if(item.Text == "")
			{
				dom.className = "toolbar_btn";
				IMCore.Utility.AttachButtonEvent(dom, "toolbar_btn", "toolbar_btn_hover", "toolbar_btn_hover");
			}
			else
			{
				dom.className = "toolbar_text_btn";
				IMCore.Utility.AttachButtonEvent(dom, "toolbar_text_btn", "toolbar_text_btn_hover", "toolbar_text_btn_hover");
			}
			dom.innerHTML = String.format("<div>{0}</div>", IMCore.Utility.ReplaceHtml(item.Text), Toolbar.PredefineHeight);
	...
	{
		IMCore.Utility.DisableSelect(This.GetDom(), true);
	}
	...
	{
		m_Btns[index].firstChild.innerHTML = IMCore.Utility.ReplaceHtml(text);
	}
...
function HorizSplit(config)
{
	...
	if(config.TopMinHeight == undefined) config.TopMinHeight = 50;
	if(config.BottomMinHeight == undefined) config.BottomMinHeight = 50;
	...
	IMCore.Utility.AttachEvent(
		...
		function(evt)
		{
			...
			MoveData = {
				...
				Bound: {
					Top: config.TopControl.GetTop() + config.TopMinHeight,
					Bottom: config.BottomControl.GetTop() + config.BottomControl.GetHeight() - config.BottomMinHeight
				},
	...
	return new Module.HorizSplit(
		{
			...
			TopMinHeight:tmh,BottomMinHeight:bmh,
...
function SimpleTabControl(config)
{
	...
	This.Reset=function(tabs)
	{
		...
		{
			var tabConf = tabs[i];
			var tab = new Tab(tabConf.Text, IMCore.Utility.IsNull(tabConf.Width, 0));
			m_Tabs.push(tab);
	...
	This.OnSelectedTab = new IMCore.Delegate();
	...
	This.SetTabVisible = function(index, visible)
	{
		if(m_Tabs.length < 2) return;
		
		if(!visible && m_SelectedTab == m_Tabs[index])
		{
			This.Select(index == 0 ? 1 : index - 1);
		}
		
		m_Tabs[index].SetVisible(visible);
	}
	
	This.Reset(IMCore.Utility.IsNull(config.Tabs,[]));
	...
	function Tab(text, width)
	{
		...
		var dom = document.createElement("DIV");
		dom.innerHTML = IMCore.Utility.ReplaceHtml(text);
