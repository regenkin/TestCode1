...
IMCore.Link("StyleSheet", IMCore.GetUrl("Module/Developer/StyleSheet.css"), "text/css");
...
'{PAD}{TAB}{{\r\n'+
'{PAD}{TAB}{TAB}m_Task=Taskbar.AddTask(This,IMCore.Utility.IsNull(config.Title.InnerHTML,""));\r\n'+
...
function init(completeCallback, errorCallback)
{
	function LoadModulesComplete()
	{
		...
		Window = IMCore.GetModule("Window.js").Window;
...
function RedirectCssUrl(dir, css)
{
	return css.replace(
		...
		function(u, url)
		{
			return String.format("url(\"{2}/{0}?UID={1}\")", url, IMCore.GenerateUniqueId(), dir);
		}
	);
}
...
{
	return IMCore.GetBrowser()=="IE"?"cur":"gif";
}
...
function GetOffset(evt, obj)
{
	...
	{
		var client = IMCore.Utility.GetClientCoord(obj);
...
function Post(callback, errorCallback, url, data, contentType)
{
	...
	if(request)
	{
		...
		request.setRequestHeader('Content-Type', IMCore.Utility.IsNull(contentType, 'application/x-www-form-urlencoded'))
...
function CodeBuilder(application) //CodeBuilder
{
	...
	'<br/>\r\n'+
	'<pre class="sh_javascript">\r\n'+
	'{PAD}IMCore.Link("StyleSheet", IMCore.GetUrl("{CSS}"), "text/css");\r\n'+
	'{PAD}\r\n'+
	...
	'{PAD}{TAB}{{\r\n'+
	'{PAD}{TAB}{TAB}Controls = IMCore.GetModule("Controls.js");\r\n'+
	'{PAD}\r\n'+
	'{PAD}{TAB}{TAB}Control = IMCore.GetModule("Controls.js").Control;\r\n'+
	'{PAD}\r\n'+
	'{PAD}{TAB}{TAB}_init(completeCallback, errorCallback);\r\n'+
	'{PAD}{TAB}}\r\n'+
	'{PAD}\r\n'+
	'{PAD}{TAB}IMCore.LoadModules(\r\n'+
	...
	'{PAD}{TAB}catch (ex)\r\n'+
	'{PAD}{TAB}{{\r\n'+
	'{PAD}{TAB}{TAB}errorCallback(new IMCore.Exception(ex.mame, ex.message));\r\n'+
	'{PAD}{TAB}}\r\n'+
	'{PAD}}\r\n'+
	'</pre>\r\n'+
	'{PAD}\r\n'+
	...
	{
		this.InitCode = IMCore.Utility.IsNull(data.InitCode, Default.InitCode);
		this.DisposeCode = IMCore.Utility.IsNull(data.DisposeCode, Default.DisposeCode);
		this.ModuleGlobalCode = IMCore.Utility.IsNull(data.ModuleGlobalCode, Default.ModuleGlobalCode);
...
function PlaceHolderBase(config)
{
	...
	This.Count = IMCore.Utility.IsNull(config.Count, 0);
	This.MaxIndex = 10;
	This.OnChanged = new IMCore.Delegate();
	This.OnFocus = new IMCore.Delegate();
	This.OnPropertyChanged = new IMCore.Delegate();

	This.HasSubControls = false;

	This.OnPHDblClick = new IMCore.Delegate();
...
function PlaceHolder(config)
{
	...
	var m_BorderWidth = IMCore.Utility.IsNull(config.ResizeBorderWidth,4);
...
function FormPH(config)
{
	...
	This.MemberCode = IMCore.Utility.IsNull(config.MemberCode, "");
	This.CustomConfigCode = IMCore.Utility.IsNull(config.CustomConfigCode, "");
	
	var m_Css = IMCore.Utility.IsNull(config._Css, "window");
	var m_HasMinBtn = IMCore.Utility.IsNull(config.HasMinButton, true);
	var m_HasMaxBtn = IMCore.Utility.IsNull(config.HasMaxButton, true);
	var m_Resizable = IMCore.Utility.IsNull(config.Resizable, true);
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				...
				{
					NAME: This.GetName(),
					CONFIG: IMCore.Utility.RenderJson(fconfig),
					CTRLS: ctrlHtml.join("\r\n")
				}
			)
		);
...
function CustomControlDesignPH(config)
{
	...
	var BaseName=IMCore.Utility.IsNull(config.BaseName,"Control");
	
	var CustomConfig=IMCore.Utility.IsNull(config.CustomConfig,"");
	var GlobalCode=IMCore.Utility.IsNull(config.GlobalCode,"");
	...
	This.RenderCode = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(Format(
			FormCodeFormat, paddingStr, paddingEM,
			{
				NAME: This.GetName(), CONFIG: IMCore.Utility.RenderJson(fconfig), CTRLS: subs.join("\r\n"),
				WIDTH: This.GetWidth(),
				HEIGHT: This.GetHeight(),
...
function FramePH(config)
{
	...
	This.OnResizedCode = IMCore.Utility.IsNull(config.OnResizedCode, "");
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, "");
	This.CustomConfigCode =  IMCore.Utility.IsNull(config.CustomConfigCode, "");
	var m_Css = IMCore.Utility.IsNull(config._Css, "control");
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				...
				paddingStr, paddingEM,
				{ NAME: This.GetName(), FULLNAME: This.GetFullName(), CONFIG: IMCore.Utility.RenderJson(fconfig) }
			)
		);
	...
	This.GetCustomConfig = function()
	{
		...
		var config = {
			_Css: m_Css,OnResizedCode: This.OnResizedCode, InitCode: This.InitCode, CustomConfigCode: This.CustomConfigCode,
			Controls:controls
		};
...
function CustomControlObjectPH(config)
{
	...
	This.ConfigCode = IMCore.Utility.IsNull(config.ConfigCode, "");
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, "");
	var m_Css = IMCore.Utility.IsNull(config._Css, "control");
	var m_ClassName = IMCore.Utility.IsNull(config.ClassName, "Control");
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				ControlCodeHtmlFormat,
				paddingStr, paddingEM,
				{ NAME: This.GetName(), FULLNAME: This.GetFullName(), CONFIG: IMCore.Utility.RenderJson(fconfig),CLASS:m_ClassName}
			)
		);
...
function ButtonPH(config)
{
	...
	This.OnClickCode = IMCore.Utility.IsNull(config.OnClickCode, "");
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, "");

	var m_Css = IMCore.Utility.IsNull(config._Css, "button");
...
function LabelPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "label");
...
function TextBoxPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "textbox");
...
function TextAreaPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "textarea");
...
function CheckBoxPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "checkbox");
...
function RadioButtonPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "radio");
	var m_Group=IMCore.Utility.IsNull(config.Group,"");
...
function ListBoxPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "listbox");
...
function DropDownListPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "dropdownlist");
...
function ListViewPH(config)
{
	...
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, String.format(DefaultCode.InitCode,This.GetName()));
	This.ListItem_Member=IMCore.Utility.IsNull(config.ListItem_Member,String.format(DefaultCode.ListItem_Member));
	This.ListItem_GetText=IMCore.Utility.IsNull(config.ListItem_GetText,String.format(DefaultCode.ListItem_GetText));

	var m_Css = IMCore.Utility.IsNull(config._Css, "listview");
	var m_BorderWidth=IMCore.Utility.IsNull(config._BorderWidth,1);
	var m_Columns=IMCore.Utility.IsNull(
		config._Columns,
		[
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				CodeHtmlFormat,
				paddingStr, paddingEM,
				{ NAME: This.GetName(), FULLNAME: This.GetFullName(), CONFIG: IMCore.Utility.RenderJson(fconfig), COLUMNS:IMCore.Utility.RenderJson(m_Columns)}
			)
		);
	}
	...
	This.RenderCode = function(builder, paddingStr, paddingCount)
	{
		...
		builder.push(Format(
			CodeFormat, paddingStr, paddingCount,
			{
				...
				COLUMNS:IMCore.Utility.RenderJson(m_Columns)
			}
		));
	}
...
function TreeViewPH(config)
{
	...
	This.OnClickCode = IMCore.Utility.IsNull(config.OnClickCode, "");
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, String.format("{0}.Refresh(function(){});",This.GetName()));
	This.DSMemberCode=IMCore.Utility.IsNull(config.DSMemberCode,"");
	This.DSGetSubNodesCode=IMCore.Utility.IsNull(config.DSGetSubNodesCode,"callback([]);");

	var m_Css = IMCore.Utility.IsNull(config._Css, "treeview");
	var m_BorderWidth=IMCore.Utility.IsNull(config._BorderWidth,1);
...
function RichEditorPH(config)
{
	...
	var m_Css = IMCore.Utility.IsNull(config._Css, "richEditor");
	var m_BorderWidth=IMCore.Utility.IsNull(config._BorderWidth,1);
...
function SimpleTabControlPH(config)
{
	function TabsSettingForm()
	{
        ...
        m_BtnAdd.OnClick.Attach(
            function(btn)
            {
				appendItem(new m_ListTabs_ListItem("新标签",100,IMCore.GenerateUniqueId()));
            }
        );
	...
	var m_DefaultTabs = [
		{ Text: "标签1", Width: 80, IsSelected: true, ID: IMCore.GenerateUniqueId() },
		{ Text: "标签2", Width: 80, IsSelected: false, ID: IMCore.GenerateUniqueId() }
	];
	...
	else
	{
		...
		{
			m_Tabs[i].ID=IMCore.GenerateUniqueId();
	...
	This.OnSelectedTabCode = IMCore.Utility.IsNull(config.OnSelectedTabCode, "");
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, "");
	var m_Css = IMCore.Utility.IsNull(config._Css, "simple_tab");
	var m_BorderWidth=IMCore.Utility.IsNull(config._Broderwidth,1);
...
function ToolbarPH(config)
{
	...
	This.OnCommandCode = IMCore.Utility.IsNull(config.OnCommandCode, "");
	This.InitCode = IMCore.Utility.IsNull(config.InitCode, "");
	var m_Css = IMCore.Utility.IsNull(config._Css, "toolbar");
	var m_Items=IMCore.Utility.IsNull(config._Items,[{ Css: "", Text: "Button", Command: "Cmd" }]);
...
function CssEditor(config)
{
	...
	This.Load = function(config)
	{
		...
		m_Editor.SetText(IMCore.Utility.IsNull(config.CssCode, m_DefalutCode));
	}
...
function CodeEditor(config)
{
	...
	var m_OnInit=IMCore.Utility.IsNull(config.OnInit,null);
...
function OpenSolutionForm()
{
    ...
    {
        m_Task = Taskbar.AddTask(This, IMCore.Utility.IsNull(config.Title.InnerHTML, ""));
    ...
    var label2 = new Controls.Label(
		{
			...
			"AnchorStyle": Controls.AnchorStyle.Left | Controls.AnchorStyle.Right | Controls.AnchorStyle.Top,
			"Parent": This,
			"Text": String.format("1.将以下代码保存到网站服务器上（请使用UTF-8编码）;<br/>2.将Css保存到\"{0}\";<br/>3.调用IMCore.Exec(function(){},alert,'代码路径.js')运行。", css),
			"Css": "label"
		}
    );
    ...
    var m_LabelInfo = new Controls.Label(
		{
			...
			Text: IMCore.Utility.IsNull(config.Tip, ""),
			Parent: This
		}
    );
    ...
    var m_EditPrompt = new Controls.TextBox(
		{
			...
			AnchorStyle: Controls.AnchorStyle.Left | Controls.AnchorStyle.Right | Controls.AnchorStyle.Bottom,
			Text: IMCore.Utility.IsNull(config.DefaultValue, ""),
			Parent: This
		}
    );
	...
	m_EditPath.GetTextBoxDom().onkeyup = function()
	{
		label4.SetText(
			String.format(
				...
				"代码文件：<span style='color:Blue;'>Core/{0}.js</span><br/>" +
...
function Instance()
{
	...
	{
		return IMCore.Path.GetFileNameNoExtention(m_SolutionFile);
	}
	...
	{
		return String.format("Core/{0}.js", m_SolutionFile);
	}
	
	function GetBinFullPath()
	{
		return IMCore.Path.GetDirectoryName(m_SolutionFile) + "/Bin";
	}
	...
	function MainForm()
	{
			...
			dialog.ShowDialog(
				...
				function()
				{
					...
					var postData = String.format(
						...
						"</Data>",
						dialog.GetPath(),
						IMCore.Utility.TransferCharForXML(IMCore.Utility.RenderJson(data))
					);
					...
					try
					{
						Post(
							function(xml, text)
							{
								...
								var ret = IMCore.Utility.ParseJson(text);
								if(ret.Result == "OK")
								{
							...
							"Lesktop.aspx", postData, "text/xml"
						);
					}
					catch(ex)
					{
					...
					function(xml, text)
					{
						...
						if (ret.Result == "OK")
						{
							...
							try
							{
								...
								var data = IMCore.Utility.ParseJson(ret.Data.Solution);
		...
		function Save(completedCallback, errorCallback)
		{
			function render()
			{
				...
				var content = IMCore.Utility.RenderJson(data);
				return content;
			}
			...
			var postData = String.format(
				...
				"</Data>",
				m_SolutionFile,
				IMCore.Utility.TransferCharForXML(content),
				IMCore.Utility.TransferCharForXML(code),
				IMCore.Utility.TransferCharForXML("")
			);
