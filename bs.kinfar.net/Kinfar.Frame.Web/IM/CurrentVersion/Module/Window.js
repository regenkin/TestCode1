...
function init(completeCallback,errorCallback)
{
	IMCore.LoadModules(
		...
		{				
			...
			AnchorStyle=IMCore.GetModule("Controls.js").AnchorStyle;
			completeCallback();
		},
		errorCallback,
		["Controls.js"]
...
function Window(config)
{
	...
	var baseConfig=IMCore.Utility.Clone(config);
	...
	This.OnHided=new IMCore.Delegate();
	This.onclosing=new IMCore.Delegate();
	...
	if(config.Title!=undefined)
	{
		...
		if(config.Title.InnerHTML!=undefined) _title.InnerHTML=IMCore.Utility.ReplaceHtml(config.Title.InnerHTML);
	}
	
	var m_ControlMode = IMCore.Utility.IsNull(config.ControlMode, false);

	var _id=IMCore.GenerateUniqueId();
	var dom=This.GetDom();
	dom.tabIndex=-1;
	...
	var m_MinHeight=IMCore.Utility.IsNull(config.MinHeight,300);
	var m_MinWidth=IMCore.Utility.IsNull(config.MinWidth,400);
	...
	var m_BkImage = new Controls.HtmlControlBkImage(
		IMCore.Utility.IsNull(config.BkImage,{ Horiz: [6, 100, 6], Vertical: [24, 100, 6], Css: "window_bk" })
	);
	
	m_BkImage.GetDom().style.position="absolute";
	m_BkImage.GetDom().style.zIndex="0";
	
	if(_title.Height == 0) domTitle.style.display = "none";
	...
	domTitle.childNodes[1].innerHTML = IMCore.Utility.ReplaceHtml(_title.InnerHTML);
	...
	var _client=new Control(
		{
			...
			BorderWidth:0,
			Parent:This,
			Css:IMCore.Utility.IsNull(config.ClientCss,"")
		}
	);
	...
	var _waiting=new Control(
		{
			...
			BorderWidth:0,
			Parent:This,
			Css:IMCore.Utility.IsNull(config.Waiting,"waiting")
		}
	);
	...
	This.Resize(IMCore.Utility.IsNull(config.Width,600),IMCore.Utility.IsNull(config.Height,450));
