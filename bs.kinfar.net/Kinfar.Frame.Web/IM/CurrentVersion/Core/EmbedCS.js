...
if (__embed_config.Init == undefined)
{
	...
	window.StartChat = function(peer, tab)
	{
		...
		var url = __embed_config.Format(
			__embed_config.Config.ServiceUrl + "/CustomService.aspx?Peer={0}&Source={1}&Name={3}&Nickname={4}&random={2}",
			peer,
			__embed_config.EncodeReferrer(document.referrer == "" ? document.location.toString() : document.referrer),
			(new Date()).getTime(),
			window.__embedcs_name == undefined ? "" : window.__embedcs_name,
			window.__embedcs_nickname == undefined ? "" : window.__embedcs_nickname
		);
