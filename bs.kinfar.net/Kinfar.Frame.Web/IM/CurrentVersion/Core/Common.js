...
if (window.IMCore == undefined) window.IMCore = {};

(function(){

IMCore.Params = (function()
{
...
IMCore.Utility.IsNull = function()
{
...
IMCore.Utility.AttachEvent = function(elem, evtName, handler)
{
	if (elem.attachEvent)
	{
...
IMCore.Utility.DetachEvent = function(elem, evtName, handler)
{
	if (elem.detachEvent)
	{
...
IMCore.Utility.AttachButtonEvent = function(elem, normal_css, hover_css, press_css)
{
...
IMCore.Utility.Clone = function(val)
{
	if (val == null)
	{
		return null
	...
	else if (val.constructor == Array)
	{
		...
		for (i in val)
		{
			a[i] = IMCore.Utility.Clone(val[i])
		}
		return a
	}
	else if (val.constructor == Object)
	{
		...
		for (c in val)
		{
			a[c] = IMCore.Utility.Clone(val[c])
		}
		return a
	}
...
IMCore.Utility.DisableSelect = function(elem, disableChildren)
{
	...
	if (IMCore.GetBrowser() == "IE")
	{
		...
		if (disableChildren)
		{
			...
			{
				IMCore.Utility.DisableSelect(elem.childNodes[i], true);
			}
		}
	}
}

IMCore.Utility.GetButton = function(evt)
{
...
IMCore.Utility.IsTextNode = function(node)
{
...
function _ClearHtml(builder, node)
{
	for (var i = 0; i < node.childNodes.length; i++)
	{
		...
		if (IMCore.Utility.IsTextNode(n))
		{
...
IMCore.Utility.ClearHtml = function(node)
{
	var builder = [];
...
IMCore.Utility.GetInnerHTML = function(nodes)
{
	var builder = [];
	for (var i = 0; i < nodes.length; i++)
	{
		if (!IMCore.Utility.IsTextNode(nodes[i]))
		{
...
IMCore.Utility.EncodeUrl = function(url)
{
	var temp = [];
...
IMCore.Utility.GetClientCoord = function(obj)
{
	...
	else
	{
		if (IMCore.GetBrowser() == "IE")
		{
			...
			{
				var offset = IMCore.Utility.GetClientCoord(offsetParent);
...
IMCore.Utility.PreventDefault = function(evt)
{
...
IMCore.Utility.CancelBubble = function(evt)
{
	if (evt && evt.stopPropagation) evt.stopPropagation();
	else evt.cancelBubble = true;
}

IMCore.Utility.GetTarget = function(evt)
{
...
IMCore.Utility.TransferCharForXML = function(str)
{
	var res = str.replace(
...
IMCore.Utility.TransferCharForJavascript = function(s)
{
...
IMCore.Utility.ReplaceHtml = function(text)
{
...
IMCore.Exception = function(name, msg)
{
	this.Name = name;
	this.Message = msg;
...
IMCore.GenerateUniqueId = function()
{
	m_Count++;
	return 'ID' + m_Count;
}

IMCore.Post = function(url, data, type, timeout, handler)
{
	try
	{
		var request = null;
			...
			if (request.readyState == 4)
			{
				try
				{
					switch (request.status)
					{
						case 200:
							{
								IMCore.Session.WriteLog(String.format("Post Success: Url = {1}, status = {0}, ResponseText = '{2}'", request.status, url, request.responseText));
						...
						default:
							{
								...
								IMCore.Session.WriteLog(String.format("Post Error : status = {0}, statusText = {1}, Url = {2}", request.status, request.statusText, url));
								break;
							}
					}
				}
		...
		return {
			Abort: function()
			{
				...
				if (request != null)
				{
					IMCore.Session.WriteLog(String.format("Post Abort: Url = {0}", url));
					request.onreadystatechange = function() { };
					request.abort();
					request = null;
					handler.onabort();
	...
	{
		handler.onerror(new IMCore.Exception(ex.name, ex.message));
	}
}
...
var ParseContructors = {
	...
	{
		return new IMCore.Exception(value.Name, value.Message);
	}
};

IMCore.Utility.ParseJson = function(str, contructors)
{
	try
	{
		var val = JSON.parse(
...
IMCore.Utility.RenderJson = function(val)
{
	if (val == null)
	{
		return null
	...
	else if (val.constructor == Array)
	{
		...
		for (var index in val)
		{
			...
			builder.push(IMCore.Utility.RenderJson(val[index]));
		}
		builder.push("]");
		return builder.join("");
	}
	else if (val.constructor == Object)
	{
		...
		{
			if (index > 0) builder.push(",");
			builder.push(String.format("\"{0}\":{1}", key, IMCore.Utility.RenderJson(val[key])));
			index++;
		}
		builder.push("}");
		return builder.join("");
	...
	{
		return String.format('"{0}"', IMCore.Utility.TransferCharForJavascript(val));
	}
	else if (val.constructor == Date)
	{
...
IMCore.Path = function()
{
}

IMCore.Path.GetFullPath = function(path)
{
	return String.format("/{0}/{1}", IMCore.Session.GetUserName(), path);
}

IMCore.Path.GetRelativePath = function(parent, sub)
{
...
IMCore.Path.GetFileName = function(fullName)
{
...
IMCore.Path.GetFileExtension = function(fullName)
{
	var index = fullName.lastIndexOf(".")
...
IMCore.Path.GetDirectoryName = function(fullName)
{
	var index = fullName.lastIndexOf("/")
	switch (index)
	{
...
IMCore.Path.GetFileNameNoExtention = function(fullName)
{
	var index = fullName.lastIndexOf("/")
	var name = (index == -1 ? fullName : fullName.substring(index + 1, fullName.length));
