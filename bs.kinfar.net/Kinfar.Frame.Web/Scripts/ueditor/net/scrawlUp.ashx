...
public class scrawlImgUp : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (action == "tmpImg")
        {
            ...
            ApplicationObject.CurrentHttpContext.Response.Write("<script>parent.ue_callback('" + info["url"] + "','" + info["state"] + "')</script>");//回调函数
        }
        else
        {
            ...
            //向浏览器返回json数据
            ApplicationObject.CurrentHttpContext.Response.Write("{'url':'" + info["url"] + "',state:'" + info["state"] + "'}");
        }
    }
