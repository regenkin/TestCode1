...
function CheckSerialNumberValue() {
    ...
    if (parseInt(serialNumber) <= 0 || parseInt(serialNumber) > 9) {
        ...
        $('#SerialNumber').focus();
        top.$.messager.alert("提示", "流水号应该在1-9之间");
        return false;
    }
    return true;
}
...
function CheckSNLengthValue() {
    ...
    if (parseInt(sNLength) <= 0 || parseInt(sNLength) > 9) {
        ...
        $('#SNLength').focus();
        top.$.messager.alert("提示", "流水号长度应该在1-9之间");
        return false;
    }
    return true;
}
...
function CheckPlaceHolderValue() {
    ...
    if (parseInt(placeHolder) < 0 || parseInt(placeHolder) > 9) {
        ...
        $('#PlaceHolder').focus();
        top.$.messager.alert("提示", "序列号长度应该在0-9之间");
        return false;
    }
    return true;
}
...
function ConstructRuleFormat(newValue, fieldName) {
    ...
    $("#RuleFormat").textbox("setText", ruleFormat);
}
...
function OverMainModuleDataHandleBeforeSaved(data) {
    var v = $('#RuleFormat').textbox('getText');
    data.push({ name: 'RuleFormat', value: v });
}