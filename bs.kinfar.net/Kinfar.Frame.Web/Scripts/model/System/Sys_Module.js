...
function OnFieldTypeSelected(record) {
    ...
    switch (record.id) {
        ...
        case 'guid':
            {
                controlType.push({ id: 8, text: '外键类型' });
...
function OverOnEditFormTabSelect(title, index) {
    if (title == '模块字段') {
        ...
        if (!moduleName || !tableName) {
            top.showAlertMsg("提示", "请先设置模块名称和模块表名的值！", "info", function () {
...
function ReCreateModule() {
    ...
    top.showConfirmMsg('提示', msg, function (action) {
        if (action) {
            $.ajax({
                type: 'post',
                dataType: 'json',
                ...
                beforeSend: function () {
                    top.openWaitDialog('模块正在重新生成...');
                },
                success: function (result) {
                    if (!result) return;
                    if (result.Success) {
                        top.showMsg('提示', '模块生成成功！', function () {
                            top.closeWaitDialog();
                            top.closeDialog();
                            top.location.reload();
                        });
                ...
                error: function () {
                    top.showAlertMsg('提示', '模块重新生成失败，服务器异常！', "error", function () {
                        top.closeWaitDialog();
                    });
                }
            });
