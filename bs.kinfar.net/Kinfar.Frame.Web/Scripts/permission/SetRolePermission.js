...
function TreeOnLoadSuccess(node, data) {
    if (data && data.length > 0) {
        ...
        $('#roleTree').tree("select", $("#" + treeDomId));
    }
}
...
function LoadRoleUser(roleId) {
    ...
    url += "&where=" + $.base64.encode("Id IN(SELECT Sys_UserId FROM dbo.Sys_UserRole WHERE Sys_RoleId='" + roleId + "')");
    options.url = url;
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Success) { //保存成功
                top.showMsg('提示', '角色权限保存成功！', function () {
                    top.closeWaitDialog();
                ...
                top.showAlertMsg('提示', '角色权限保存失败，' + result.Message, "error", function () {
                    top.closeWaitDialog();
                });
            }
        },
        error: function (err) {
            top.showAlertMsg('提示', '角色权限保存失败，服务器异常！', "error", function () {
                top.closeWaitDialog();
            });
        },
        dataType: "json"
