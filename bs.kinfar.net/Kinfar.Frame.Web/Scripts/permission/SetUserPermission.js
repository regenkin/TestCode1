...
$(function () {
    if (!tempUserId) { //从菜单进入
        //加载用户树
        LoadTree($('#userTree'), '/' + CommonController.Async_Data_Controller + '/GetTreeByNode.html?icon=eu-icon-user&moduleName=' + encodeURI("用户管理"));
        //设置右边tab左边框
        $("#tabs_permission").parent().css('border-left-width', '1px').css('border-left-style', 'solid').css('border-left-color', top.getBorderColor());
    }
    else { //从用户列表进入
...
function TreeOnLoadSuccess(node, data) {
    if (data && data.length > 0) {
        ...
        if (userId == '00000000-0000-0000-0000-000000000000' && data[0].children && data[0].children.length > 0) { //第一个节点为非正常根节点
            userId = data[0].children[0].id;
            treeDomId = "_easyui_tree_2";
        }
        LoadUserPermission(userId);
        $('#userTree').tree("select", $("#" + treeDomId));
    }
}
...
function SetDataPermissionRange(obj, type) {
    ...
    var toolbar = [{
        ...
        iconCls: "eu-icon-ok",
        handler: function (e) {
            var obj = top.getCurrentDialogFrame()[0].contentWindow.GetSelectedOrgs();
            //获取网格tr的id前缀
    ...
    title += "权限范围设置";
    top.openDialog(title, url, toolbar, 500, 520, 'eu-icon-crog');
}
...
function SetFieldPermission(obj, type) {
    ...
    var toolbar = [{
        ...
        iconCls: "eu-icon-ok",
        handler: function (e) {
            var obj = top.getCurrentDialogFrame()[0].contentWindow.GetSelectFields();
            //获取网格tr的id前缀
            var getPermissionGridPrefix = function () {
                var tr = $("div.datagrid-view2 div.datagrid-body tr.datagrid-row").eq(0);
                var tempId = tr.attr("id");
    ...
    title += "权限设置";
    top.openDialog(title, url, toolbar, 800, 400, 'eu-icon-crog');
}
...
function SaveUserPermission(backFun) {
    ...
    if (!menuIds || menuIds.length == 0) {
        top.showAlertMsg('提示', '没有权限数据！', 'info');
        return;
    }
    var powerData = [];
    ...
    $.ajax({
        ...
        data: data,
        beforeSend: function () {
            top.openWaitDialog('权限保存中...');
        },
        success: function (result) {
            if (result.Success) { //保存成功
                top.showMsg('提示', '用户权限保存成功！', function () {
                    top.closeWaitDialog();
                ...
                top.showAlertMsg('提示', '用户权限保存失败，' + result.Message, "error", function () {
                    top.closeWaitDialog();
                });
            }
        },
        error: function (err) {
            top.showAlertMsg('提示', '用户权限保存失败，服务器异常！', "error", function () {
                top.closeWaitDialog();
            });
        },
        dataType: "json"
