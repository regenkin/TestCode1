...
function AddNewFlow() {
    ...
    if (node && node.id) {
        ...
        var url = "/Page/EditForm.html?page=add&moduleName=流程信息";
        top.openDialog('新建流程', url, toolbar, 650, 280, 'eu-icon-add');
    }
}
...
function AddFolder() {
    var toolbar = [{
        ...
        iconCls: "eu-icon-ok",
        handler: function (e) {
            top.getCurrentDialogFrame()[0].contentWindow.Save(null, function (result) {
    ...
    top.openDialog('新建目录', url, toolbar, 650, 200, 'eu-icon-edit');
}
...
function DeleteNode() {
    ...
    if (node && node.id) {
        ...
        top.showConfirmMsg('提示', '确定要删除' + moduleName + '【' + node.text + '】吗?', function (action) {
            if (action) {
...
function LoadFlow() {
    ...
    if (node && node.id) {
        ...
        if (workflowId) {
            ...
            top.openDialog('编辑流程', url, toolbar, 650, 280, 'eu-icon-edit');
        }
    }
}
