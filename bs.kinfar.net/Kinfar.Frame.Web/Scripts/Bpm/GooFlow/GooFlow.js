...
function GooFlow(bgDiv, property) {
    ...
    this.$title = bgDiv.attr('title') || "newFlow_1";//流程图的名称
    ...
    if (property.haveHead) {
        ...
        for (var x = 0; x < property.headBtns.length; ++x) {
            ...
            tmp += "<a href='javascript:void(0)' title='" + tips + "' class='GooFlow_head_btn'><i class='ico_" + property.headBtns[x] + "'></i></a>"
        }
        tmp += "</div>";
        this.$head = $(tmp);
        ...
        this.onBtnImportClick = null;//导入流程图按钮定义
        this.onBtnExportClick = null;//导出流程图按钮定义
        ...
        if (property.headBtns)
            this.$head.on("click", { inthis: this }, function (e) {
                ...
                switch ($(tar).attr("class")) {
                    ...
                    case "ico_import": if (This.onBtnImportClick != null) This.onBtnImportClick(); break;
                    case "ico_export": if (This.onBtnExportClick != null) This.onBtnExportClick(); break;
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            var typeName = type.replace(' round', '');
            var nodeName = This.$nodeRemark[typeName];
            if (!nodeName) nodeName = typeName + "_" + This.$max;
            This.addNode(This.$id + "_" + typeName + "_" + This.$max, { name: nodeName, left: X, top: Y, type: This.$nowType }, 'tool');
            This.$max++;
        });
        //划线或改线时用的绑定
        ...
        this.onItemNodeDbClick = null; //结点双击事件
        this.onItemLineDbClick = null; //连线双击事件触发
...
GooFlow.prototype = {
    ...
    initDraw: function (id, width, height) {
        ...
        $(this.$draw).delegate(tmpClk, "dblclick", { inthis: this }, function (e) {
            ...
            if (This.onItemLineDbClick != null && typeof (This.onItemLineDbClick) == 'function') {
                This.onItemLineDbClick(tempId);
                return;
            }
    ...
    addNode: function (id, json, from) {
        ...
        if (this.onItemAddBakFun != null && typeof (this.onItemAddBakFun) == 'function') {
            this.onItemAddBakFun(id, json.type, from);
        }
    },
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item > .span", "dblclick", { inthis: this }, function (e) {
            ...
            var This = e.data.inthis;
            if (This.onItemNodeDbClick != null && typeof (This.onItemNodeDbClick) == 'function') {
                This.onItemNodeDbClick(id);
                return;
            }
    ...
    setName: function (id, name, type) {
        ...
        if (type == "node") {//如果是结点
            ...
            if (this.$nodeData[id].type.indexOf("round") > 1) {
                this.$nodeDom[id].find(".span").text(name);
            }
            else {
    ...
    //删除结点
    delNode: function (id, flag) {
        if (!this.$nodeData[id]) return;
        if (!flag) { //默认启用删除事件，否则直接删除
            if (this.onItemDel != null && !this.onItemDel(id, "node")) return;
        }
        var workNodeId = this.$nodeDom[id].attr('worknodeid');//流程节点id
        //if (this.onItemDel != null && !this.onItemDel(id, "node")) return;
        //先删除可能的连线
        ...
        if (this.onItemDelBakFun != null && typeof (this.onItemDelBakFun) == 'function') {
            this.onItemDelBakFun(workNodeId);
        }
    },
    //设置流程图的名称
