...
function AddRoleForm(obj) {
    ...
    if (roleId != undefined && roleId != null && roleId.length > 0) {
        var toolbar = [{
            ...
            iconCls: "eu-icon-ok",
            handler: function (e) {
                top.openWaitDialog('表单保存中...');
                top.getCurrentDialogFrame()[0].contentWindow.SaveForm(function (result) {
                    top.closeWaitDialog();
        ...
        top.openDialog('角色【' + roleName + '】－新增【' + moduleName + '】表单', url, toolbar, 660, 540);
    }
    else {
        top.showAlertMsg('提示', '找不到对应的角色！');
    }
}
...
function EditRoleForm(obj) {
    ...
    if (formId == undefined || formId == null || formId.length <= 0) {
        top.showAlertMsg('提示', '请选择表单后再点击编辑表单按钮！');
        return;
    }
    var roleId = $(obj).attr("roleId");
    if (roleId != undefined && roleId != null && roleId.length > 0) {
        ...
        var moduleName = $("#txtModule").combobox("getText");
        var roleName = $(obj).attr("roleName");
        top.openDialog('角色【' + roleName + '】－编辑【' + moduleName + '】表单', url, toolbar, 630, 540);
    }
    else {
        top.showAlertMsg('提示', '找不到对应的角色！');
    }
...
function DelRoleForm(obj) {
    ...
    var roleName = $(obj).attr("roleName");
    top.showConfirmMsg("删除角色表单", "确定要删除角色【" + roleName + "】的模块表单【" + formName + "】吗？", function (ok) {
        if (ok) {
            var formId = $("#txtForm").combobox("getValue");
            if (formId == undefined || formId == null || formId.length <= 0) {
                top.showAlertMsg('提示', '请选择表单后再点击删除表单按钮！');
                return;
            }
            var roleId = $(obj).attr("roleId");
            if (roleId != undefined && roleId != null && roleId.length > 0) {
                top.openWaitDialog('数据处理中...');
                $.ajax({
                    type: 'post',
                    ...
                    success: function (result) {
                        top.closeWaitDialog();
                        if (result && result.Success) {
                            top.showMsg('提示', '角色【' + roleName + '】的模块表单【' + formName + '】删除成功！', function () {
