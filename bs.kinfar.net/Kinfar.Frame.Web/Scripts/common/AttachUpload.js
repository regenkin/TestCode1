...
function AddUpFile() {
    ...
    var addHtml = "<a id='btnAdd" + length + "' class='easyui-linkbutton' style='margin-right:5px;' iconCls='eu-icon-add' name='a_Add'></a>";
    var removeHtml = "<a id='btnRemove" + length + "' class='easyui-linkbutton' style='margin-right:5px;' iconCls='eu-icon-del' name='a_Remove'></a>";
...
//moduleId:模块Id
//backFun:上传成功后的回调函数
function UploadFile(moduleId,backFun) {
    ...
    if (sameFiles.length > 0) {
        top.showAlertMsg("提示", "文件【" + sameFiles + "】已经在上传列表中，请不要重复上传！", "error");
        return;
    }
    ...
    $("#form_Upload").ajaxSubmit({
        ...
        error: function (err) {
            top.closeWaitDialog();
            top.showAlertMsg('提示', '附件上传失败，服务端异常！', 'error');
        },
        dataType: "json"
    });
}
