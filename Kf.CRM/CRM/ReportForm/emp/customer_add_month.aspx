...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            items.push({ type: 'textbox', id: 'smonth', text: '月份：' });
        ...
        function initSelect() {
            ...
            $("#smonth").ligerComboBox({
                width: 60, data: smonthDate, initValue: nowMonth
            });
        ...
        function doserch() {
            //var sendtxt = "&Action=emp_customer&idlist=" + $("#tree1_val").val() + "&rnd=" + Math.random();
            ...
            var columns = [], t = {};
            t.Rows = [], tt = {};
            columns.push({ display: '编号', width: 40, render: function (item, i) { return i + 1; } });
            columns.push({ display: '员工姓名', name: 'name', width: 120, render: function (item) { if (item.name == "") item.name = "未分类";return item.name; }});  
            for (i = 1; i <= daycount; i++)
            {
                columns.push({ display: i, name: "i", width: 30, render: function (item) { if ((item.m1)) return item.i; else return "0"; } });
                t.Rows.push({ name: i, i: i });

            }
            $("#maingrid5").ligerGetGridManager().reColumns(columns);
            f_reload();
        }
        function f_reload() {
            var manager = $("#maingrid5").ligerGetGridManager();
