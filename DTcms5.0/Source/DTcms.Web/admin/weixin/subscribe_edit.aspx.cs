...
namespace DTcms.Web.admin.weixin
{
    public partial class subscribe_edit : Web.UI.ManagePage
    {
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("subscribe_edit.aspx", "action={0}&id={1}", this.action, ddlAccountId.SelectedValue));
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            else if (rblResponseType.SelectedItem.Value == "2") //语音
            {
                ...
                ls.Add(new Model.weixin_request_content()
                {
                    account_id = this.id,
                    rule_id = ruleId,
                    title = txtSoundTitle.Text.Trim(),
                    media_url = txtSoundUrl.Text.Trim(),
                    content = txtSoundContent.Text.Trim()
                });
                model.contents = ls;
            }
            //判断是新增还是修改
