...
namespace DTcms.Web.MVC.ERPAjax
{
   /// <summary>
   /// submit_ajax 的摘要说明
   /// </summary>
   public class submit_ajax : IHttpHandler, IRequiresSessionState {
      ...
      public void ProcessRequest(HttpContext context) {
         ...
         switch (action) {
            ...
            case "bc.get_article_goods_info": //商品信息
      ...
      private void comment_add(HttpContext context) {
         ...
         Model.users userModel = new BaseController().GetUserInfo();
         if (userModel != null) {
      ...
      private void validate_username(HttpContext context) {
        ...
        private void user_login(HttpContext context)
        {
            ...
            string corp = DTRequest.GetFormString("txtCorp");
            ...
            if (model.status == 1) //待验证
            {
                if (userConfig.regverify == 1)
                {
                    context.Response.Write("{\"status\":1, \"url\":\""
                        + new BaseController().getlink(sitepath, new BaseController().linkurl("register", "?action=sendmail&username=" + Utils.UrlEncode(model.user_name))) + "\", \"msg\":\"会员尚未通过验证！\"}");
                }
                else
                {
                    context.Response.Write("{\"status\":1, \"url\":\"" +
                        new BaseController().getlink(sitepath, new BaseController().linkurl("register", "?action=sendsms&username=" + Utils.UrlEncode(model.user_name))) + "\", \"msg\":\"会员尚未通过验证！\"}");
                }
                return;
            }
            ...
            {
                context.Response.Write("{\"status\":1, \"url\":\""
                    + new BaseController().getlink(sitepath, new BaseController().linkurl("register", "?action=verify&username=" + Utils.UrlEncode(model.user_name))) + "\", \"msg\":\"会员尚未通过审核！\"}");
                return;
            }
            //检查用户每天登录是否获得积分
      ...
      private void user_check_login(HttpContext context) {
         //检查用户是否登录
         Model.users model = new BaseController().GetUserInfo();
         if (model == null) {
      ...
      private void user_register(HttpContext context) {
         ...
         if (userConfig.regstatus == 3) {
            ...
            context.Response.Write("{\"status\":1, \"msg\":\"注册成功，请进入邮箱验证激活账户！\", \"url\":\""
                + new BaseController().getlink(sitepath, new BaseController().linkurl("register", "?action=sendmail&username=" + Utils.UrlEncode(model.user_name))) + "\"}");
         }
         //需要人工审核
         else if (userConfig.regverify == 1) {
            context.Response.Write("{\"status\":1, \"msg\":\"注册成功，请等待审核通过！\", \"url\":\""
                + new BaseController().getlink(sitepath, new BaseController().linkurl("register", "?action=verify&username=" + Utils.UrlEncode(model.user_name))) + "\"}");
         }
         else {
            context.Session[DTKeys.SESSION_USER_INFO] = model;
            context.Session.Timeout = 45;
            ...
            context.Response.Write("{\"status\":1, \"msg\":\"注册成功，欢迎成为本站会员！\", \"url\":\""
                + new BaseController().getlink(sitepath, new BaseController().linkurl("usercenter", "index")) + "\"}");
         }
         return;
      }
      #endregion
      ...
      private void user_avatar_crop(HttpContext context) {
         ...
         //取得保存的新文件名
         DTcms.Web.MVC.UI.UpLoad upFiles = new DTcms.Web.MVC.UI.UpLoad();
      ...
      private void user_getpassword(HttpContext context) {
         ...
         if (type.ToLower() == "mobile") //使用手机取回密码
            ...
            context.Response.Write("{\"status\":1, \"msg\":\"手机验证码发送成功！\", \"url\":\""
                + new BaseController().getlink(sitepath, new BaseController().linkurl("repassword", "?action=mobile&username=" + Utils.UrlEncode(model.user_name))) + "\"}");
            return;
            #endregion
         }
            ...
            bodytxt = bodytxt.Replace("{linkurl}", "http://" + HttpContext.Current.Request.Url.Authority.ToLower()
                + new BaseController().getlink(sitepath, new BaseController().linkurl("repassword", "?action=email&code=" + codeModel.str_code)));
            //发送邮件
            try {
      ...
      private void user_amount_recharge(HttpContext context) {
         ...
         context.Response.Write("{\"status\":1, \"msg\":\"订单保存成功！\", \"url\":\""
             + new BaseController().getlink(sitepath, new BaseController().linkurl("payment", "?action=confirm&order_no=" + order_no)) + "\"}");
         return;
      ...
      private void cart_goods_add(HttpContext context) {
         ...
         //检查商品是否正确
         if (goods_id > 0 && !new BLL.article_goods().Exists(channel_id, article_id, goods_id)) {
            context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品参数有误！\"}");
            return;
         }
         //查找会员组
         int group_id = 0;
         Model.users groupModel = new BaseController().GetUserInfo();
         if (groupModel != null) {
            group_id = groupModel.group_id;
         }
         //统计购物车
         DTcms.Web.MVC.UI.Controllers.ShopCart.Add(channel_id, article_id, goods_id, quantity);
         Model.cart_total cartModel = DTcms.Web.MVC.UI.Controllers.ShopCart.GetTotal(group_id);
      ...
      private void cart_goods_update(HttpContext context) {
         ...
         Model.cart_keys model = DTcms.Web.MVC.UI.Controllers.ShopCart.Update(channel_id, article_id, goods_id, quantity);
         if (model != null) {
            context.Response.Write("{\"status\":1, \"msg\":\"商品数量修改成功！\", \"channel_id\":" + model.channel_id + ", \"article_id\":" + model.article_id + ", \"goods_id\":" + model.goods_id + ", \"quantity\":" + model.quantity + "}");
            return;
         }
      ...
      private void cart_goods_delete(HttpContext context) {
         ...
         int goods_id = DTRequest.GetFormInt("goods_id", 0);
         if (clear == 1) {
            DTcms.Web.MVC.UI.Controllers.ShopCart.Clear(); //清空购物车
         ...
         DTcms.Web.MVC.UI.Controllers.ShopCart.Clear(channel_id, article_id, goods_id);
      ...
      private void order_save(HttpContext context) {
         ...
         foreach (Model.cart_items item in goodsList) {
            //检查库存数量
            if (new BLL.article().GetStockQuantity(item.channel_id, item.article_id, item.goods_id) < item.quantity) {
         ...
         //删除购物车对应的商品
         DTcms.Web.MVC.UI.Controllers.ShopCart.Clear(iList);
         //清空结账清单
         Utils.WriteCookie(DTKeys.COOKIE_SHOPPING_BUY, "");
         //提交成功，返回URL
         context.Response.Write("{\"status\":1, \"url\":\""
             + new BaseController().getlink(sitepath, new BaseController().linkurl("payment", "?action=confirm&order_no=" + model.order_no)) + "\", \"msg\":\"恭喜您，订单已成功提交！\"}");
         return;
      }
      #endregion
      ...
      private void view_cart_count(HttpContext context) {
         context.Response.Write("document.write('" + DTcms.Web.MVC.UI.Controllers.ShopCart.GetQuantityCount() + "');");
      }
      #endregion
      ...
      private string send_verify_email(string sitepath, Model.users userModel) {
         ...
         bodytxt = bodytxt.Replace("{valid}", userConfig.regemailexpired.ToString());
         bodytxt = bodytxt.Replace("{linkurl}", "http://" + HttpContext.Current.Request.Url.Authority.ToLower()
             + new BaseController().getlink(sitepath, new BaseController().linkurl("register", "?action=checkmail&code=" + codeModel.str_code)));
         //发送邮件
         try {
            DTMail.sendMail(sysConfig.emailsmtp, sysConfig.emailssl,
                sysConfig.emailusername,
