...
namespace DTcms.Web.api.payment.balance
{
    public partial class index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (modelorders == null)
            {
               Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("error", "?msg=" + Utils.UrlEncode("对不起，订单详情获取出错,请重试！")));
                return;
            }
            ...
            //检查是否已登录
            Model.users userModel = new Web.MVC.UI.Controllers.BaseController().GetUserInfo();
            if (userModel == null)
            {
               Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("payment", "?action=login")); //尚未登录
                return;
            }
            if (userModel.amount < order_amount)
            {
               Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("payment", "?action=recharge")); //账户的余额不足
                return;
            }
            ...
            if (order_no.StartsWith("B")) //B开头为商品订单
            {
                ...
                if (model == null)
                {
                   Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("error", "?msg=" + Utils.UrlEncode("对不起，商品订单号不存在！")));
                    return;
                }
                if (model.payment_status == 1)
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        if (!result1)
                        {
                           Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("payment", "?action=error"));
                            return;
                        }
                        //扣除积分
                        if (model.point < 0)
                ...
                //支付成功
                Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("payment", "?action=succeed&order_no=" + order_no));
                return;
            }
            Response.Redirect(new Web.MVC.UI.Controllers.BaseController().linkurl("error", "?msg=" + Utils.UrlEncode("对不起，找不到需要支付的订单类型！")));
            return;
        }
    }
}