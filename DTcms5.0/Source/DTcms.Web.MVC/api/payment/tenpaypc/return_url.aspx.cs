         ...
         if (site_payment_id > 0) {
            ...
            if (resHandler.isTenpaySign()) {
               if ("0".Equals(trade_state)) {
                  ...
                  Response.Redirect(new DTcms.Web.MVC.UI.Controllers.BaseController().linkurl("payment", "?action=succeed&order_no=" + out_trade_no));
                  return;
               }
            }
         }
         //认证签名失败
         Response.Redirect(new DTcms.Web.MVC.UI.Controllers.BaseController().linkurl("payment", "?action=error"));
         return;
      }
   }
}