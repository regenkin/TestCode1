...
//显示评论(初始加载前10条)
function AjaxPageList(listDiv, pageIndex, pageSize, pageCount, sendUrl, defaultAvatar) {
   //pageIndex -页面索引
   //pageSize -每页显示条数
   //pageCount -取得总页数
   InitComment(pageIndex);//初始化评论数据
      ...
      $.ajax({
         ...
         success: function (data) {
            ...
            for (var i in data) {
               ...
               '<div class="pl_user_name">' + data[i].user_name + '</div>' +
               '<div class="pl_user_time">' + getTimeSpan(data[i].add_time) + '</div></div>' +
					'<div class="pl_user_comment">' + unescape(data[i].content) + '</div>';
