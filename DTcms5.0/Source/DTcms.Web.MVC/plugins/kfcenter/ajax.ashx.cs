    ...
    public class ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void site_export(HttpContext context)
        {
            ...
            {//遍历xml配置文件中table标签,逐个表处理
                ...
                //        DataSet ds = null;
                //        //ds= bll.Query("select * from " + article_table);
        ...
        {
            //读入站点备份配置文件
            string xmlpath = Utils.GetMapPath("~/plugins/kfcenter/DTcms5.0db_table.xml");
        ...
        private void site_import(HttpContext context)
        {
            ...
            //    if (!dsData.Tables.Contains(table_name) && table_name != "{" + sysConfig.sysdatabaseprefix + "article}")
            //    {//如果备份数据中没有当前表，则跳过
            //        continue;
            //    }
            ...
            //    if (table_name == "{" + sysConfig.sysdatabaseprefix + "article}")
            //    {//处理通道文章表
            //        foreach (DataRow row in dsData.Tables[channel_table_name].Rows)
            //        {//遍历通道表，每个通道都一个对应的文章表
        ...
        private void TestDB(HttpContext context)
        {
            try
            {
                ...
                Newtonsoft.Json.Linq.JToken jtxtDBServerName = jobject["txtDBServerName"];
            ...
            {
                DTcms.Web.MVC.Areas.admin.LogHelper.ErrorLog("DTcms.Web.Mvc.Plugin.KfCenter.ajax.TestDB", exp);
                context.Response.Write("{\"status\": 0,\"msg\": \""+exp.Message+ "\"}");
            }
            finally
            {
