...
namespace DTcms.Web.MVC.tools
{
   ...
   public class download : IHttpHandler, IRequiresSessionState {
      ...
      public void ProcessRequest(HttpContext context) {
         ...
         //获得下载ID
         if (id == 0) {
            context.Response.Redirect(new Web.MVC.UI.Controllers.BaseController().getlink(sitepath,
                new Web.MVC.UI.Controllers.BaseController().linkurl("error", "?msg=" + Utils.UrlEncode("出错了，文件参数传值不正确！"))));
            return;
         }
         //检查下载记录是否存在
         ...
         if (!bll.Exists(id)) {
            context.Response.Redirect(new Web.MVC.UI.Controllers.BaseController().getlink(sitepath,
                new Web.MVC.UI.Controllers.BaseController().linkurl("error", "?msg=" + Utils.UrlEncode("出错了，您要下载的文件不存在或已经被删除！"))));
            return;
         }
         ...
         if (model.point > 0) {
            ...
            if (userModel == null) {
               //自动跳转URL
               HttpContext.Current.Response.Redirect(new Web.MVC.UI.Controllers.BaseController().getlink(sitepath, new Web.MVC.UI.Controllers.BaseController().linkurl("login")));
            }
            //如果该用户未曾下载过该附件
            if (!bll.ExistsLog(model.id, userModel.id)) {
               ...
               if (model.point > userModel.point) {
                  context.Response.Redirect(new Web.MVC.UI.Controllers.BaseController().getlink(sitepath,
                      new Web.MVC.UI.Controllers.BaseController().linkurl("error", "?msg=" + Utils.UrlEncode("出错啦，您的积分不足支付本次下载！"))));
                  return;
               }
               //扣取积分
