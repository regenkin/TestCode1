...
namespace DTcms.Web.MVC.tools
{
   ...
   public class admin_ajax : IHttpHandler, IRequiresSessionState {
      ...
      public void ProcessRequest(HttpContext context) {
         //检查管理员是否登录
         if (!new DTcms.Web.MVC.UI.Controllers.ManageController().IsAdminLogin()) {
      ...
      private void site_path_validate(HttpContext context) {
         ...
         BLL.sites_extension bll = new BLL.sites_extension();
      ...
      private void get_navigation_list(HttpContext context) {
         Model.manager adminModel = new DTcms.Web.MVC.UI.Controllers.ManageController().GetAdminInfo();//获得当前登录管理员信息
         if (adminModel == null) {
            return;
         }
         ...
         // 修改原asp.net导航路径为MVC格式路径 
         foreach (DataRow dr in dt.Rows) {
            string link_url = dr["link_url"].ToString();
            if (link_url.IndexOf("/") > 0) {
               link_url = "admin" + link_url.Substring(link_url.IndexOf("/"));
            }
            dr["link_url"] = "/" + link_url.Replace(".aspx", "/index");
         }
      ...
      private void edit_order_status(HttpContext context) {
         //取得管理员登录信息
         Model.manager adminInfo = new DTcms.Web.MVC.UI.Controllers.ManageController().GetAdminInfo();
         if (adminInfo == null) {
