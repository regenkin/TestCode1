            ...
            foreach (Model.url_rewrite model in SiteUrls.GetUrls().Urls) {
               ...
               if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1)) {
                  ...
                  result = string.Format("{0}{1}/{2}{3}",
                      siteConfig.webpath, DTKeys_Extension.DIRECTORY_REWRITE_MVC, sitePath, requestPage);
            ...
            foreach (Model.url_rewrite model in SiteUrls.GetUrls().Urls) {
               ...
               if (model.url_rewrite_items.Count == 0 &&
                  ...
                  result = string.Format("{0}{1}/{2}",
                      siteConfig.webpath, "web", model.page.Replace(".cshtml", "/index"));
                  context.RewritePath(result);
                  return;
               }
               //遍历URL字典的子节点
               foreach (Model.url_rewrite_item item in model.url_rewrite_items) {
                  ...
                  if (Regex.IsMatch(requestPage, string.Format("^/{0}$", newPattern), RegexOptions.None | RegexOptions.IgnoreCase)
                     ...
                     if (siteConfig.staticstatus == 2 && (model.channel.Length > 0 || model.page.ToLower() == "index/index")) {
                        //频道页
                        result = siteConfig.webpath + DTKeys.DIRECTORY_REWRITE_HTML + "/" + sitePath +
