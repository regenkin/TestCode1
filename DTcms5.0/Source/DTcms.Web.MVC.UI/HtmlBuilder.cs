    ...
    public class HtmlBuilder
    {
       ...
       private const string urlstr = "\"{0}tools/admin_ajax.ashx?action=get_builder_html&lang={1}&html_filename=&indexy=&aspx_filename={2}&catalogue={3}\"";
        ...
        public void getpublishsite(HttpContext context)
        {
            ...
            List<DTcms.Model.url_rewrite> ls = (!string.IsNullOrEmpty(type)) ? bll.GetList(name, type) : bll.GetList(name);
            string linkurl = string.Empty;
            sbjson.Append("[");
            if (type == "indexlist")
            {
                #region 针对特殊需求
               List<DTcms.Model.url_rewrite> ls2 = (!string.IsNullOrEmpty("list")) ? bll.GetList(name, "list") : bll.GetList(name, "list");
                foreach (DTcms.Model.url_rewrite modeltrewrite2 in ls2)
                {
                    if (modeltrewrite2.url_rewrite_items.Count > 0)
                    {
                        ...
                        {
                            //遍历URL字典的子节点
                           foreach (DTcms.Model.url_rewrite_item item2 in modeltrewrite2.url_rewrite_items)
                            {
            ...
            else
            {
               ...
               foreach (DTcms.Model.url_rewrite modeltrewrite in ls)
                {
                    if (modeltrewrite.url_rewrite_items.Count > 0)
                    {
                        ...
                        {
                            //遍历URL字典的子节点
                           foreach (DTcms.Model.url_rewrite_item item in modeltrewrite.url_rewrite_items)
                            {
                                if (item.querystring == string.Empty)
                                {
                                    linkurl = string.Format("{0}/{1}/{2}", DTKeys_Extension.DIRECTORY_REWRITE_MVC, lang, modeltrewrite.page);
        ...
        private string GetArticleIndexUrlList(string lang, string channelname, string page, string pattern, string path, string querystring, int pagesize)
        {
           ...
           for (int q = 1; q <= pageindex; q++) {
              ...
              string linkurl = string.Format("{0}/{1}/{2}?{3}", DTKeys_Extension.DIRECTORY_REWRITE_MVC, lang, page, querystr);
