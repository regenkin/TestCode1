...
namespace DTcms.Web.MVC.UI.Controllers
{
   ...
   public partial class ShopCart {
      ...
      /// 添加到购物车
      /// </summary>
      public static bool Add(int channel_id, int article_id, int goods_id, int quantity) {
         List<Model.cart_keys> ls = GetCart();
         if (ls != null) {
            Model.cart_keys modelt = ls.Find(p => p.channel_id == channel_id && p.article_id == article_id && p.goods_id == goods_id);
            if (modelt != null) {
               int i = ls.FindIndex(p => p.channel_id == channel_id && p.article_id == article_id && p.goods_id == goods_id);
         ...
         //不存在的则新增
         ls.Add(new Model.cart_keys() { channel_id = channel_id, article_id = article_id, goods_id = goods_id, quantity = quantity });
      ...
      /// 更新购物车数量
      /// </summary>
      public static Model.cart_keys Update(int channel_id, int article_id, int goods_id, int quantity) {
         //如果数量小于1则移除该项
         if (quantity < 1) {
            return null;
         }
      ...
      /// 移除购物车指定项
      /// </summary>
      public static void Clear(int channel_id, int article_id, int goods_id) {
         if (channel_id > 0 && article_id > 0) {
            List<Model.cart_keys> cartList = GetCart();
            if (cartList == null) {
               return;
            }
            Model.cart_keys modelt = cartList.Find(p => p.channel_id == channel_id && p.article_id == article_id && p.goods_id == goods_id);
            if (modelt != null) {
      ...
      public static void Clear(List<Model.cart_keys> ls) {
         if (ls != null) {
            ...
            foreach (Model.cart_keys modelt in ls) {
               Model.cart_keys model = cartList.Find(p => p.channel_id == modelt.channel_id && p.article_id == modelt.article_id && p.goods_id == modelt.goods_id);
               if (model != null) {
            ...
            if (ls != null) {
               foreach (Model.cart_keys modelt in ls) {
                  ...
                  if (modelt.goods_id > 0) {
                     isExists = new BLL.article_goods().Exists(modelt.channel_id, modelt.article_id, modelt.goods_id);
                  }
                  else {
                     isExists = new BLL.article().Exists(modelt.channel_id, modelt.article_id);
                  }
                  if (isExists) {
