...
namespace DTcms.API.Weixin.Common
{
    ...
    {
        Model.sysconfig sysConfig = new BLL.sysconfig().loadConfig(); //站点配置
        ...
        public IResponseMessageBase GetResponseMessageeMusic(RequestMessageText requestMessage, int ruleId, int accountId)
        {
            ...
            if (model == null)
            {
                new BLL.weixin_response_content().Add(accountId, requestMessage.FromUserName, requestMessage.MsgType.ToString(), requestMessage.Content, "music", "-1", requestMessage.ToUserName);
            }
            else
            {
                responseMessage.Music.Title = model.title;
                responseMessage.Music.Description = model.content;
                responseMessage.Music.ThumbMediaId = model.media_id;
                if (model.media_url.ToLower().StartsWith("http://"))
                {
                ...
                else
                {
                    ...
                    responseMessage.Music.MusicUrl = sysConfig.weburl + "/" + sysConfig.webpath.Replace("/", "") + model.media_url;
                }
                if (!string.IsNullOrEmpty(model.meida_hd_url))
                {
                    if (model.meida_hd_url.ToLower().StartsWith("http://"))
                    {
                        responseMessage.Music.HQMusicUrl = model.meida_hd_url;
                    }
                    else
                    {
                        //因为安装目录是以/开头，所以去掉，以免出现双斜杆
                        responseMessage.Music.HQMusicUrl = sysConfig.weburl + "/" + sysConfig.webpath.Replace("/", "") + model.meida_hd_url;
                    }
                }
                new BLL.weixin_response_content().Add(accountId, requestMessage.FromUserName, requestMessage.MsgType.ToString(), requestMessage.Content, "music", "音乐链接：" + model.media_url + "|标题：" + model.title + "|描述：" + model.content, requestMessage.ToUserName);
            }
            return responseMessage;
        }
        ...
        public IResponseMessageBase GetResponseMessageNews(RequestMessageText requestMessage, int ruleId, int accountId)
        {
            ...
            foreach (Model.weixin_request_content modelt in twList)
            {
                ...
                else
                {
                    ...
                    {
                        //因为安装目录是以/开头，所以去掉，以免出现双斜杆
                        article.PicUrl = sysConfig.weburl + "/" + sysConfig.webpath.Replace("/", "") + modelt.img_url;
                    }
                }
                artList.Add(article);
            }
