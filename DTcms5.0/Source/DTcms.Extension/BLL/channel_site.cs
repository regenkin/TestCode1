...
namespace DTcms.BLL {
   ...
   /// 系统频道表
   /// </summary>
   public partial class site_channel_exp {
      private readonly Model.sysconfig sysConfig = new BLL.sysconfig().loadConfig();//获得系统配置信息
      private readonly DAL.site_channel dal;
      private const string DIRECTORY_REWRITE_MVC = "Areas/Web/Views";
      
      public site_channel_exp() {
      ...
      public bool Exists(string name) {
         ...
         if (DirPathExists(sysConfig.webpath + "/" + DIRECTORY_REWRITE_MVC + "/", name)) {
            return true;
         }
         //与存在的频道名称是否同名
         if (dal.Exists(name)) {
