...
{
   /// <summary>
   /// 插件管理,与DTcms.BLL.plugin功能相同,只是dal调用的是plugin_extension
   /// </summary>
    public partial class plugin_extension
    {
        private readonly Model.sysconfig sysConfig = new BLL.sysconfig().loadConfig();//获得站点配置信息
        private readonly DAL.plugin_extension dal;

        public plugin_extension()
        {
            dal = new DAL.plugin_extension(sysConfig.sysdatabaseprefix);
        }
