...
namespace DTcms.BLL {
   /// <summary>
   /// 系统频道表(DTcms.BLL.site_channel中引用了aspx路径,在这个类中修正为引用Areas/web/views路径)
   /// </summary>
   public partial class site_channel_extension {
      private readonly Model.sysconfig sysConfig = new BLL.sysconfig().loadConfig();//获得系统配置信息
      private readonly DAL.site_channel dal;
      
      public site_channel_extension() {
      ...
      public bool Exists(string name) {
         ...
         //与站点MVC视图目录下的一级文件夹是否同名
         if (DirPathExists(sysConfig.webpath + "/" + DTKeys_Extension.DIRECTORY_REWRITE_MVC + "/", name)) {
            return true;
         }
         //与存在的频道名称是否同名
         if (dal.Exists(name)) {
