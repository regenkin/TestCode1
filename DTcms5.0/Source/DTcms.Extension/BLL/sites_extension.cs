...
namespace DTcms.BLL {
   ...
   public partial class sites_extension {
      ...
      public bool Update(Model.sites model) {
         ...
         if (dal.Update(model, old_build_path)) {
            if (old_build_path.ToLower() != model.build_path.ToLower()) {
               //更改频道分类对应的目录名称
               FileHelper.MoveDirectory(sysConfig.webpath + DTKeys_Extension.DIRECTORY_REWRITE_MVC + "/" + old_build_path,
                   sysConfig.webpath + DTKeys_Extension.DIRECTORY_REWRITE_MVC + "/" + model.build_path);
      ...
      public bool Exists(string build_path) {
         ...
         if (DirPathExists(sysConfig.webpath + "/" + DTKeys_Extension.DIRECTORY_REWRITE_MVC + "/", build_path)) {
            return true;
         }
         //与频道名称是否重名
