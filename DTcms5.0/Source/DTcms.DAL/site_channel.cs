            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.is_spec > 0)
                        {
                            new DAL.navigation(databaseprefix).Add(conn, trans, newNavId, "channel_" + model.name + "_spec", "规格管理", "article/spec_list.aspx", 102, model.id, "Show,View,Add,Edit,Delete");
                        }
                        //开启评论则新增菜单
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        //规格导航菜单处理
                        if (model.is_spec > 0 && oldModel.is_spec == 0)
                        {
                            //如开启规格而以前关闭的需新增菜单
                            new DAL.navigation(databaseprefix).Add(conn, trans, "channel_" + model.name, "channel_" + model.name + "_spec", "规格管理", "article/spec_list.aspx", 102, model.id, "Show,View,Add,Edit,Delete");
                        }
                        else if (model.is_spec == 0 && oldModel.is_spec > 0)
                        {
                            //如关闭规格而以前开启需删除菜单
                            new DAL.navigation(databaseprefix).Delete(conn, trans, "channel_" + oldModel.name + "_spec");
                        }
                        else if (model.is_spec > 0)
                        {
                            //如都是开启状态则修改菜单
                            new DAL.navigation(databaseprefix).Update(conn, trans, "channel_" + oldModel.name + "_spec", "channel_" + model.name + "_spec");
                        }
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        string article_table_name = databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channelName;
                        if (DbHelperSQL.TabExists(article_table_name)) {
                           strSql2.Append("drop table " + article_table_name);
